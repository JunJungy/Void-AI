  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans">
      ...
      <main className="pl-64 pb-28 pt-8 px-12">
        <h1 className="text-4xl font-display font-bold mb-8">Explore</h1>
        ...
        <section className="mb-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {CATEGORIES.map(cat => (
              <div key={cat.name} className={`h-32 rounded-xl bg-gradient-to-br ${cat.color} p-4 flex items-end relative overflow-hidden group cursor-pointer hover:scale-105 transition-transform`}>
                <div className="absolute inset-0 bg-black/20 group-hover:bg-black/0 transition-colors" />
                <span className="font-bold text-white relative z-10">{cat.name}</span>
              </div>
            ))}
          </div>
        </section>
